subroutine recom_nitogenss(tr_num, tracers, partit, mesh) 
  ...
  Real(kind=WP)                     :: tv, Fc
  real(kind=WP)                     :: Vben(mesh%nl),  aux(mesh%nl), LocDenit(partit%myDim_nod2D+partit%eDim_nod2D)
...
if (mype==0 .and. (mod(mstep,100)==0))  print *, achar(27)//'[37m'//'         --> recom_nitogenss'//achar(27)//'[0m'
   ...
   do n=1, myDim_nod2D ! needs exchange_nod in the end
      ...
      do nz=nlevels_nod2D_minimum, nl1
         tv = tracers%data(10)%values(nz,n)*Vben(nz) !idetc [mmolC m-2 d-1]
!         tv = tr_arr(nz,n,10)*Vben(nz) !idetc [mmolC m-2 d-1]
      ...
      nz=nl1
      tv = tracers%data(10)%values(nz,n)*Vben(nz) !idetc
