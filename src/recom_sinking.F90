...
module integrate_bottom_interface
  interface
    subroutine integrate_bottom(tflux, partit, mesh)
        use mod_mesh
        USE MOD_PARTIT
        USE MOD_PARSUP
        type(t_partit), intent(inout), target :: partit
        ...
        do k=nzmin, nzmax
           ...
           !do tr_num=1,num_tracers
              ...
              if (use_density_scaling) then
                 if (tracers%data(tr_num)%ID ==1008)then
                    if (tracers%data(tr_num)%values(k,row)>0.001) then ! only apply ballasting above a certain biomass
                       scaling_density1_3D(k,row) = (rho_particle1(k,row)-rho_seawater(1))/(rho_ref_part-rho_ref_water)
                    endif 
                 endif
#if defined (__3Zoo2Det)

                    if (tracers%data(tr_num)%ID ==1026)then
                       if (tracers%data(tr_num)%values(k,row)>0.001) then ! only apply ballasting above a certain biomass
...
!-------------------------------------------------------------------------------  
subroutine get_particle_density(tracers, partit, mesh) ! NEW BALL developed by Cara and Onur  
...
subroutine get_seawater_viscosity(tr_num, tracers, partit, mesh)!,Nn,Temp,Salt,seawater_visc,mesh) ! NEW BALL developed by Cara and Onur    
...
!           print*,'seawater_visc_3D(k,row)= ',seawater_visc_3D(k,row)
     enddo
  end do
