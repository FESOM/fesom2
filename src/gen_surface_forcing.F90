...
#if defined (__recom)
!   USE g_forcing_arrays,    only: GloFeDust, GloNDust, RiverDIN2D, RiverDON2D, RiverDOC2D, RiverDSi2D, RiverAlk2D, RiverDIC2D !OG
!   USE g_forcing_arrays,    only: ErosionTSi2D, ErosionTON2D, ErosionTOC2D !OG
   use recom_config
...
#include "associate_mesh_ass.h"
      ...
      if (iost == 0) then
         if (mype==0) WRITE(*,*) '     file   : ', 'namelist_recom for sbc',' open ok'
      else
         if (mype==0) WRITE(*,*) 'ERROR: --> bad opening file   : ', 'namelist_recom for sbc',' ; iostat=',iost
         call par_ex(partit%MPI_COMM_FESOM, partit%mype)
         stop
      endif
...
#if defined (__recom)
      character(15):: CO2vari, Nvari !OG
      integer      :: firstyearofcurrentCO2cycle, totnumyear, currentCO2year !OG
      character(4) :: currentCO2year_char !OG
      real(kind=8), allocatable :: ncdata(:) !OG
...
! ******** Riverine input (Nutrients) *********
         ...
         ! add river nutrients as surface boundary condition (surface_bc function in oce_ale_tracers)
            ...
            is_riverinput = 0.0d0
            if (mype==0) write(*,*) 'No riverine input' 
         end if
...
! ******** Erosion (Nutrients) *********
         ...
         is_erosioninput = 1.0d0
	       ...
	       ErosionTSi2D=ErosionTON2D * 16/15
            ...
            if (mype==0) write(*,*) 'No erosion input' 
         end if
