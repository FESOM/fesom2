...
! REcoM's own SBC namelist (namelist.recom / nam_rsbc) is read separately by
! sbc_ini_recom, which is called from forcing_setup right after this routine.
! It used to be declared and read inline here; see sbc_ini_recom below.
...
#include "associate_mesh_ass.h"
      ...
      if (l_xstre) then
         if (mype==0) then
            write(*,*) "      nm_xstre_file = ", trim(nm_xstre_file) ," ! name of file with X stress"
            write(*,*) "      nm_xstre_var  = ", trim(nm_xstre_var)  ," ! name of variable in file with wind stress "
         end if
         i_totfl=i_totfl+1
         i_xstre=i_totfl
      end if
      ...
      if (l_ystre) then
         if (mype==0) then
            write(*,*) "      nm_ystre_file = ", trim(nm_ystre_file) ," ! name of file with Y stress"
            write(*,*) "      nm_ystre_var  = ", trim(nm_ystre_var)  ," ! name of variable in file with wind stress "
         end if
         i_totfl=i_totfl+1
         i_ystre=i_totfl
      end if
    ...
    if (use_sw_pene) then
        ...
        end if ! --> if (chl_data_source == 'Sweeney') then
    end if ! --> if (use_sw_pene) then
    ...
    if (mype==0) write(*,*) 'Parts of forcing data (only constant in time fields) are read'
      ...
      ! REcoM's SBC namelist (namelist.recom) used to be opened and read right
      ! here. That read is now done in sbc_ini_recom (below), called from
      ! forcing_setup immediately after this routine, so REcoM's forcing init
      ! no longer needs to be interleaved with the generic sbc_ini logic.
   ...
   SUBROUTINE sbc_do(partit, mesh)
      ...
      !!----------------------------------------------------------------------
      use g_clock
      ! REcoM's own use-associations (recom_config, recom_glovar, REcoM_ciso)
      ! moved to sbc_do_recom, below, along with all REcoM forcing logic.
      IMPLICIT NONE
      ...
      logical                               :: file_exist=.false.
      ! REcoM-specific locals (CO2vari, Nvari, sediment/loopback bookkeeping,
      ! etc.) moved to sbc_do_recom, below, along with all REcoM forcing logic.
      type(t_partit), intent(inout), target :: partit
      type(t_mesh),   intent(in),    target :: mesh
...
! ============================================================
    ...
    integer                   :: CO2start, CO2count, RIstart, RIcount
...
!                               "Inorganic" 14C approximation: delta_14C := Delta_14C 
                            ...
                            AtmCO2_14(lat_zone(lat_val),:) = &
                                CO2_for_spinup * (1. + 0.001 * delta_co2_14)
                        end do
                    end if
                end if
            ...
            !if (mype==0) write(*,*) 'Updating CO2 climatology for month       ', i,' from ', trim(filename)
                ...
                firstyearofcurrentCO2cycle = lastyearoffesomcycle &
                                           - numofCO2cycles*totnumyear &
                                           + (currentCO2cycle-1) * totnumyear
                ...
                CO2start = 1
                CO2count = 12
                status   = nf90_get_var(ncid, varid, ncdata, &
        ...
        end if  ! use_atbox / prescribed CO2
...
!   Control output of atmospheric CO2 values
    ...
    ! ----------------------------------------------------------------
    if (useAeolianN) then
        ! TODO: verify behaviour when useAeolianN is .true.
        if (mstep == 1) then  ! Year has changed — update annually
    ...
    !   River inputs are in mmol/m2/s. When useRivers is set, Alk/DIC/DIN/DOC/DON
    !   are read from a monthly climatology file and DSi is derived from DIN via
    !   a Redfield-like ratio; otherwise all riverine fluxes are zeroed.
    !-----------------------------------------------------------------------------
    if (useRivers) then
    ...
    !-----------------------------------------------------------------------------
    ! Riverine iron input
    !   River runoff (m/s) is multiplied with an Fe concentration to give
    !   muemolFe/m3 -> muemolFe/m2/s, added as a surface boundary condition
    !   (surface_bc function in oce_ale_tracers).
    !-----------------------------------------------------------------------------
    if (useRivFe) then
    ...
    !=========================================================================
    ! EROSION INPUT — REcoM Biogeochemistry
    !   Applies monthly erosion fluxes as a surface boundary condition.
    !   Units: mmol m^-2 s^-1 (passed to surface_bc in oce_ale_tracers).
    !
    !   Variables loaded:
    !     POC -> ErosionTOC2D  (particulate organic carbon)
    !     PON -> ErosionTON2D  (particulate organic nitrogen)
    !     TSi -> ErosionTSi2D  (silicate, derived from TON via Redfield ratio)
    !
    !   Silicate is not stored in the erosion file; it is estimated from TON
    !   using a Redfield-like Si:N ratio of 16:15.
    !=========================================================================
    if (useErosion) then
        ...
        if (update_monthly_flag) then
            ...
            !--- Compute current month index with year-end wrap-around ---
            ! At mstep == 1 the month index is used as-is; thereafter it is
            ! incremented by 1 to advance to the next calendar month.
            i = month
            if (mstep > 1) i = i + 1
            ...
            !--- Read particulate organic tracers from the monthly file ---
            ...
            ! Silicate is absent from the erosion file; derive it from TON
            ! using a Redfield-like Si:N ratio of 16:15. Double-precision
            ! literals avoid the integer-division pitfall (16/15 = 1).
            ErosionTSi2D = ErosionTON2D * (16.0d0 / 15.0d0)
    ...
    !-----------------------------------------------------------------------------
    ! Sedimentary input (MEDUSA)
    !   Two update paths share the same underlying reads:
    !     - sedflx_num /= 0 : read once, on the very first call
    !     - sedflx_num == 0 : read once per month, via monthly_event()
    !   Both paths call the same two helpers below so the field list, unit
    !   conversion and loopback-flux logic live in exactly one place.
    !-----------------------------------------------------------------------------
    if (use_MEDUSA) then

        sedfilename = trim(ResultPath)//'medusa_flux2fesom.nc'  ! renamed via jobscript
 
        if (sedflx_num /= 0) then
            ! one-off read, first call only (GloSed is allocated with sedflx_num
            ! time-independent, so re-reading is never needed)
            do_read = (mstep == 1)
            if (do_read .and. mype==0) write(*,*) 'Updating sedimentary input from', trim(sedfilename)
        !else
        !    ! monthly-varying field: re-read at the start of every month
        ...
        if (do_read) then
            ...
            call read_medusa_sediment_fluxes(sedfilename, partit, mesh)
 
            if (add_loopback) then
                call read_medusa_loopback_fluxes(sedfilename, partit)
            end if
        end if
 
        if (mype == 0 .and. mstep == 1 .and. sedflx_num == 0) &
            write(*,*) 'sedimentary input from MEDUSA not used!'
...
! TODO: eliminate types and move it to recom folder  :OG
SUBROUTINE read_medusa_sediment_fluxes(sedfilename, partit, mesh)
      !!---------------------------------------------------------------------
      !! ** Purpose : read the MEDUSA sediment flux fields (DIN, DIC, Alk,
      !!              DSi, O2, and optionally DIC13/DIC14) into GloSed and
      !!              convert from per-day to per-second units.
      !!              Used both for the one-off read (sedflx_num /= 0) and
      !!              the monthly re-read (sedflx_num == 0).
      !!----------------------------------------------------------------------
      use recom_config
      use recom_glovar
      use REcoM_ciso
      IMPLICIT NONE
      character(len=*), intent(in)          :: sedfilename
      type(t_partit),    intent(inout), target :: partit
      type(t_mesh),      intent(in),    target :: mesh

      ! variable name -> GloSed column, only the entries actually needed are read
      character(len=8), parameter :: varnames(7) = &
         (/ 'df_din  ', 'df_dic  ', 'df_alk  ', 'df_dsi  ', 'df_o2   ', &
            'df_dic13', 'df_dic14' /)
      integer :: n, ncols
#include "associate_part_def.h"
#include "associate_mesh_def.h"
...
#include "associate_mesh_ass.h"
      ...
      ncols = 5
      if (ciso)          ncols = 6
      if (ciso.and.ciso_14) ncols = 7

      do n = 1, ncols
         call read_2ddata_on_grid_NetCDF(sedfilename, trim(varnames(n)), 1, GloSed(:,n), partit, mesh)
      end do

      ! per-day -> per-second
      GloSed(:,1:ncols) = GloSed(:,1:ncols) / 86400._WP

   END SUBROUTINE read_medusa_sediment_fluxes


   SUBROUTINE read_medusa_loopback_fluxes(sedfilename, partit)
      !!---------------------------------------------------------------------
      !! ** Purpose : read the MEDUSA loopback scalars (organic matter,
      !!              opal, CaCO3, and their isotopic variants) from the
      !!              sediment file and turn them into the lb_flux field
      !!              that is added back to the ocean through runoff.
      !!              Used both for the one-off read (sedflx_num /= 0) and
      !!              the monthly re-read (sedflx_num == 0).
      !!----------------------------------------------------------------------
      use recom_config
      ...
      ! name of each loopback scalar, in the fixed ncdata(1:9) slot order
      character(len=20), parameter :: lb_names(9) = (/ &
         'loopback_orgm_din  ', 'loopback_orgm_dic   ', 'loopback_orgm_alk   ', &
         'loopback_opal       ', 'loopback_caco3       ', &
         'loopback_orgm_dic13 ', 'loopback_caco313      ', &
         'loopback_orgm_dic14 ', 'loopback_caco314       ' /)

      integer, dimension(2)     :: istart, icount
      real(kind=8)               :: total_runoff
      real(kind=8), allocatable  :: ncdata(:)
      integer                    :: status, ncid, varid, n_lb, nvals
      ...
      nvals = 5
      if (ciso)                              nvals = 7
      if (ciso .and. ciso_14 .and. ciso_organic_14) nvals = 9
      ...
      do n_lb = 1, nvals
         status = nf_inq_varid(ncid, trim(lb_names(n_lb)), varid)
         if (status /= nf_noerr) call handle_err(status)
         status = nf_get_vara_double(ncid, varid, istart, icount, ncdata(n_lb))
         if (status /= nf_noerr) call handle_err(status)
         if (partit%mype==0) write(*,*) partit%mype, trim(lb_names(n_lb))//':', ncdata(n_lb)
      end do

      status = nf_close(ncid)

      ! sediment-outflux -> ocean-influx via runoff (mmol/m2/s)
      do n_lb = 1, 9
   ...
   END SUBROUTINE read_medusa_loopback_fluxes
END MODULE g_sbf
