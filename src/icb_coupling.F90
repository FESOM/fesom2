...
!    use i_arrays
    ...
    write(*,*) "LA DEBUG: reset ib fluxes"
    ibfwbv = 0
    ibfwb = 0
    ibfwl = 0
    ibfwe = 0
    ...
    if(i_have_element) then 
        ...
        do i=1, 3
            ...
            if (iceberg_node>0) then
                ibfwbv(iceberg_node) = ibfwbv(iceberg_node) - fwbv_flux_ib(ib) / tot_area_nods_in_ib_elem !mesh%area(1,iceberg_node) / num_ib_nods_in_ib_elem
                ibfwb(iceberg_node) = ibfwb(iceberg_node) - fwb_flux_ib(ib) / tot_area_nods_in_ib_elem !mesh%area(1,iceberg_node) / num_ib_nods_in_ib_elem
                ibfwl(iceberg_node) = ibfwl(iceberg_node) - fwl_flux_ib(ib) / tot_area_nods_in_ib_elem !mesh%area(1,iceberg_node) / num_ib_nods_in_ib_elem
                ibfwe(iceberg_node) = ibfwe(iceberg_node) - fwe_flux_ib(ib) / tot_area_nods_in_ib_elem !mesh%area(1,iceberg_node) / num_ib_nods_in_ib_elem
                ibhf(iceberg_node) = ibhf(iceberg_node) - heat_flux_ib(ib) / tot_area_nods_in_ib_elem !mesh%area(1,iceberg_node) / num_ib_nods_in_ib_elem
                !ibfwbv(iceberg_node) = ibfwbv(iceberg_node) - fwbv_flux_ib(ib) / mesh%areasvol(1,iceberg_node) / num_ib_nods_in_ib_elem
                !ibfwb(iceberg_node) = ibfwb(iceberg_node) - fwb_flux_ib(ib) / mesh%areasvol(1,iceberg_node) / num_ib_nods_in_ib_elem
                !ibfwl(iceberg_node) = ibfwl(iceberg_node) - fwl_flux_ib(ib) / mesh%areasvol(1,iceberg_node) / num_ib_nods_in_ib_elem
                !ibfwe(iceberg_node) = ibfwe(iceberg_node) - fwe_flux_ib(ib) / mesh%areasvol(1,iceberg_node) / num_ib_nods_in_ib_elem
                !ibhf(iceberg_node) = ibhf(iceberg_node) - heat_flux_ib(ib) / mesh%areasvol(1,iceberg_node) / num_ib_nods_in_ib_elem
            end if
        end do
    end if
!write(*,*) "LA DEBUG: sum(ibfwbv)=",sum(ibfwbv),"; sum(ibfwb)=",sum(ibfwb),"; sum(ibfwl)=",sum(ibfwl),"; sum(ibfwe)=",sum(ibfwe),"; sum(ibhf)=",sum(ibhf)
!write(*,*) "LA DEBUG: sum(fwbv_flux_ib)=",sum(fwbv_flux_ib),"; sum(fwb_flux_ib)=",sum(fwb_flux_ib),"; sum(fwl_flux_ib)=",sum(fwl_flux_ib),"; sum(fwe_flux_ib)=",sum(fwe_flux_ib),"; sum(heat_flux_ib)=",sum(heat_flux_ib)
