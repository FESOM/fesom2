...
!!PS             aux_ustar = sqrt( sqrt( stress_atmoce_x(node)**2 + stress_atmoce_y(node)**2 )*density_0_r ) ! @ the surface (eqn. 2)
            ...
            !!PS aux_surfbuoyflx_nl(1) = kpp_sbuoyflx(node)
            !!PS if (use_sw_pene) then
            !!PS     ! take total heatflux from shortwave radiation into account --> 
            !!PS     ! here only needed to calculate monin-obukov mixing length
            !!PS     !!PS aux_surfbuoyflx_nl(1) = aux_surfbuoyflx_nl(1)+aux_coeff*sw_3d(1,node)
            !!PS     aux_surfbuoyflx_nl(1) = aux_surfbuoyflx_nl(1)+aux_coeff*sw_3d(nun,node) 
            !!PS end if 
            ...
            call cvmix_coeffs_kpp(                     &
                ...
                !!PS w_s       = aux_ws(:),                 & ! (out)   turbulent velocity scale for tracer
                !!PS w_m       = aux_wm(:)                  & ! (out)   turbulent velocity scale for momentum
                )
