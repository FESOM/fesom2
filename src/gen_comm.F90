...
subroutine communication_nodn
  ...
  allocate(recv_from_pe(nod2D), send_to_pes(MAX_LAENDERECK,nod2D), STAT=IERR)
  if (IERR /= 0) then
  ...
  do n=1,nod2D
    if (part(n)==mype) then
    ! For every node, that belongs to the same partition mype  
           ...
           ! Over all nodes in every element el
              ...
              if (part(nod) /= mype) then
                 ...
                 if (recv_from_pe(nod)==-1) then  ! nod already collected to be received?
                    ...
                    ! nodes from which the current partition reveives from
...
! Checks, if all possible connected partition
  ...
  enddo
     ...
     if (max_laendereck_too_small) then
        print *,'Set MAX_LAENDERECK in gen_modules_partitioning.F90 to a larger value and recompile'
        stop
     endif
...
subroutine communication_elemn
  ...
  allocate(recv_from_pe(elem2D), send_to_pes(MAX_LAENDERECK,elem2D), STAT=IERR)
  if (IERR /= 0) then
