...
#if defined (__diaH) 
        locDICremax = max(locDICremax,maxval(tracers%data(43)%values(mesh%ulevels_nod2D(n):mesh%nlevels_nod2D(n)-1,n)) ) ! DICremin tracer (added by Sina)
        locDICremin = min(locDICremin,minval(tracers%data(43)%values(mesh%ulevels_nod2D(n):mesh%nlevels_nod2D(n)-1,n)) )
#endif
#endif
      end do
