...
module g_cvmix_tidal
    ...
    character(200):: tidal_botforc_vname= 'wave_dissipation'
...
#include "associate_mesh.h"
        ...
        allocate(tidal_forc_bottom_2D(elem_size))
        ...
        if (file_exist) then
            ...
            call read_other_NetCDF(tidal_botforc_file, tidal_botforc_vname, 1, tidal_forc_bottom_2D, .true., mesh, .false.) 
            !                                                                                                  |           
            ...
            do elem=1, myDim_elem2D
                ...
                if (elem2D_nodes(1,elem)<=myDim_nod2D) then
                    loc_Etot = loc_Etot + elem_area(elem)*tidal_forc_bottom_2D(elem)
                end if     
            end do
            ...
            if (tidal_botforc_Etot /= 0.0_WP) then
                tidal_forc_bottom_2D = tidal_forc_bottom_2D * tidal_botforc_Etot/glb_Etot
...
#include "associate_mesh.h"
        ...
        do elem = 1,elem_size
            ...
            zbar_e(nun)   = zbar_e(nun+1) + helem(nun ,elem)
            ...
            call cvmix_compute_Simmons_invariant_low(         &
                 nlev            = nln ,                      &
                 energy_flux     = tidal_forc_bottom_2D(elem),& !in W m-2  
                 rho             = density_0,                 &
