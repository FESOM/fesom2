  ...
  t1=MPI_Wtime()
    ...
    if (i==1 .and. action .and. istep/=1) call net_rec_from_atm(action, partit)
        ...
        elseif (i.eq.12) then
         ...
         elseif (i.eq.14) then
             if (action) then
	              runoff(:)            =  runoff(:) + exchange(:)        ! For Greenland only runoff and calving forcing from OMIP reader
    	          mask=1.
	              call force_flux_consv(runoff, mask, i, 0, action, partit, mesh)
             end if
         elseif (i.eq.15) then
             if (action) then
	              enthalpyoffuse(:)    = enthalpyoffuse(:) + exchange(:)        ! enthalpy of fusion for Greenland calving from OMIP reader
    	          mask=1.
	              call force_flux_consv(enthalpyoffuse, mask, i, 0, action, partit, mesh)
             end if
#endif 
