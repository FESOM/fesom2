subroutine recom_sinking_new(tr_num,mesh)
  ...
  type(t_mesh), intent(in) , target :: mesh
    ...
    integer       :: nz, nln, nl1, nl2, nl12, id, nzmin, nzmax, n, nn, tr_num, k, nlevels_nod2D_minimum
    integer       :: elnodes1(3), elnodes2(3), el(2), ednodes(2)
    real(kind=8) :: ttf(mesh%nl-1)
    real(kind=8) :: tvert(mesh%nl), wflux(mesh%nl)
    real(kind=WP) :: vd_flux(mesh%nl)
    real(kind=8) :: dz_trr(mesh%nl), zinv(mesh%nl-1), dz_el, aux, cflfac
    real(kind=8) :: grad_v0Eiw(2), deltaX1, deltaY1, deltaX2, deltaY2
    logical       :: debug=.false.
  ...
  Real(kind=8),dimension(mesh%nl)   :: Wvel_flux            ! Vertical velocity for fluxes
...
!if (mype==0) write(*,*) "minval= ", minval(Wvel_flux(nzmin:nzmax)), "maxval= ", maxval(Wvel_flux(nzmin:nzmax))
...
!     do nz=nzmax, 2,-1
        do nz=nzmax,nzmin+1,-1
            ...
            wflux(nz)= (0.5 * wPs * (tr_arr(nz,n,tr_num)              + psiM * Rj)+ &
...
!if (mype==0) write(*,*) "minval= ", minval(wflux(nzmin:nzmax)), "maxval= ", maxval(wflux(nzmin:nzmax))

if (1) then 
    do nz=nzmin+1,nzmax
!        tv = tr_arr(nz,n,tr_num)                                ! simple scheme - test1
!!       tv = 0.5_WP*(tr_arr(nz-1,n,tr_num)+tr_arr(nz,n,tr_num)) ! consider both layers - test2  
!       tv = tv*Wvel_flux(nz)         ! in both cases  ! Wvel_flux is negative

        tv = tr_arr(nz-1,n,tr_num)*max(Wvel_flux(nz),0._WP) + tr_arr(nz,n,tr_num)*min(Wvel_flux(nz),0._WP)  ! try upwind feel the direction :/
        vd_flux(nz)= tv*area(nz,n)
!        vd_flux(nz)= wflux(nz)*area(nz,n)
!!!       vd_flux(nz)= (area(nz,n))*(tr_arr(nz-1,n,tr_num)-tr_arr(nz,n,tr_num))*(-Wvel_flux(nz)) ! another try
    end do
...
!        tv = tr_arr(nz,n,tr_num)         ! simple scheme - test1
!        tv = tv*Wvel_flux(nz)         ! in both cases  ! Wvel_flux is negative
!        vd_flux(nz)=tv*(area(nz,n)-area(nz+1,n))
!!        vd_flux(nz)=(area(nz,n)-area(nz+1,n))* 0.d0 !wflux(nz)
!     end do
!!     nz=nzmax
!     !vd_flux(nz+1)= (area(nz+1,n))* wflux(nz)
!!     vd_flux(nz+1)= (area(nz+1,n))* 0.d0 !wflux(nz)
!!     vd_flux(nzmax+1)= 0.0d0
     !_______________________________________________________________________
     ! Surface
!     vd_flux(nzmin)= 0.0d0
!     vd_flux(nzmin)=Wvel_flux(nzmin)*tr_arr(nzmin,n,tr_num)*area(nzmin,n) !analogue to wflux but 1-d array
!     ! Zero bottom
!     vd_flux(nzmax+1)=Wvel_flux(nzmax+1)*tr_arr(nzmax,n,tr_num)*area(nzmax+1,n) !analogue to wflux but 1-d array
!     vd_flux(nzmax+1)= 0.0d0

!if (mype==0) write(*,*) "minval= ", minval(vd_flux(nzmin:nzmax)), "maxval= ", maxval(vd_flux(nzmin:nzmax))
        !_______________________________________________________________________
...
!        vert_sink(nzmax,n) = vert_sink(nzmax,n) + (vd_flux(nzmax))*dt_sink/areasvol(nzmax,n)/(zbar_3d_n(nzmax,n)-zbar_3d_n(nzmax+1,n)) ! /dz_trr(nz,n)
...
!Test with nod_integrate and see vert_sinking is ZERO 
