...
type(t_mesh), intent(inout), target :: mesh
 ...
 real(kind=WP), allocatable, dimension(:)        :: EOF_in
...
!NR #include "associate_mesh.h"
  ...
  !==============================
  allocate(mapping(chunk_size), EOF_in(chunk_size))
  ...
  IF(mype==0) write(*,*) 'mesh%elem2d', mesh%elem2d
 ...
 if (mype==0) write(*,*) 'elements are read' 
  ...
  if ((visc_option==8) .AND. (bs_eof))   then
      
      allocate(SB_l(1))
      allocate(SB_s(1))
      
      if (mype==0)  then 
         file_name=trim(meshpath)//'output1.out'
         open(fileID, file=file_name)
         read(fileID,*) SB_l
         read(fileID,*) SB_s
         write(*,*) 'reading '// trim(file_name)  
      ...
      !IF(mype==0) write(*,*) 'SB_l', size(SB_l), SB_l
      !IF(mype==0) write(*,*) 'SB_s', size(SB_s), SB_s
      
      call MPI_BCast(SB_l, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror) 
      call MPI_BCast(SB_s, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror)
      
      
      allocate(SB_EOF1(myDim_elem2D))
      !allocate(SB_EOF1(myDim_elem2D+eDim_elem2D))
      ...
      do nchunk=0, (mesh%elem2D-1)/chunk_size
          ...
          if (mype==0) then
             do n=1, k
                read(fileID,*) EOF_in(n)
             end do
          end if
          
          !IF(mype==0) write(*,*) 'EOF_in_before', EOF_in
      
          call MPI_BCast(EOF_in(1:k), k, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror)
          
          !IF(mype==0) write(*,*) 'EOF_in_after', EOF_in
  ...
  !IF(mype==0) write(*,*) 'SB_EOF1', SB_EOF1
  
      allocate(SB_l2(1))
      allocate(SB_s2(1))
      
      if (mype==0)  then 
         file_name=trim(meshpath)//'output2.out'
         open(fileID, file=file_name)
         read(fileID,*) SB_l2
         read(fileID,*) SB_s2
         write(*,*) 'reading '// trim(file_name)  
      ...
      !IF(mype==0) write(*,*) 'SB_l2', size(SB_l2), SB_l2
      !IF(mype==0) write(*,*) 'SB_s2', size(SB_s2), SB_s2
      
      call MPI_BCast(SB_l2, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror) 
      call MPI_BCast(SB_s2, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror)
      
      
      allocate(SB_EOF2(myDim_elem2D))
      ...
      !IF(mype==0) write(*,*) 'SB_EOF2', SB_EOF2
      ...
      if (mype==0)  then 
         file_name=trim(meshpath)//'output3.out'
         open(fileID, file=file_name)
         read(fileID,*) SB_l3
         read(fileID,*) SB_s3
         write(*,*) 'reading '// trim(file_name)  
      ...
      call MPI_BCast(SB_l3, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror) 
      call MPI_BCast(SB_s3, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror)
      
      
      allocate(SB_EOF3(myDim_elem2D))
      !allocate(SB_EOF1(myDim_elem2D+eDim_elem2D))
  ...
  !IF(mype==0) write(*,*) 'SB_EOF1_size', size(SB_EOF1)

      allocate(SB_l4(1))
      allocate(SB_s4(1))
      
      if (mype==0)  then 
         file_name=trim(meshpath)//'output4.out'
         open(fileID, file=file_name)
         read(fileID,*) SB_l4
         read(fileID,*) SB_s4
         write(*,*) 'reading '// trim(file_name)  
      ...
      call MPI_BCast(SB_l4, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror) 
      call MPI_BCast(SB_s4, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror)
      
      
      allocate(SB_EOF4(myDim_elem2D))
      !allocate(SB_EOF1(myDim_elem2D+eDim_elem2D))
  ...
  !IF(mype==0) write(*,*) 'SB_EOF1_size', size(SB_EOF1)
      ...
      if (mype==0)  then 
         file_name=trim(meshpath)//'output5.out'
         open(fileID, file=file_name)
         read(fileID,*) SB_l5
         read(fileID,*) SB_s5
         write(*,*) 'reading '// trim(file_name)  
      ...
      call MPI_BCast(SB_l5, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror) 
      call MPI_BCast(SB_s5, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror)
      
      
      allocate(SB_EOF5(myDim_elem2D))
      !allocate(SB_EOF1(myDim_elem2D+eDim_elem2D))
  ...
  !IF(mype==0) write(*,*) 'SB_EOF1_size', size(SB_EOF1)
      ...
      if (mype==0)  then 
         file_name=trim(meshpath)//'output6.out'
         open(fileID, file=file_name)
         read(fileID,*) SB_l6
         read(fileID,*) SB_s6
         write(*,*) 'reading '// trim(file_name)  
      ...
      call MPI_BCast(SB_l6, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror) 
      call MPI_BCast(SB_s6, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror)
      
      
      allocate(SB_EOF6(myDim_elem2D))
      !allocate(SB_EOF1(myDim_elem2D+eDim_elem2D))
  ...
  !IF(mype==0) write(*,*) 'SB_EOF1_size', size(SB_EOF1)
      ...
      if (mype==0)  then 
         file_name=trim(meshpath)//'output7.out'
         open(fileID, file=file_name)
         read(fileID,*) SB_l7
         read(fileID,*) SB_s7
         write(*,*) 'reading '// trim(file_name)  
      ...
      call MPI_BCast(SB_l7, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror) 
      call MPI_BCast(SB_s7, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_FESOM, ierror)
      
      
      allocate(SB_EOF7(myDim_elem2D))
      !allocate(SB_EOF1(myDim_elem2D+eDim_elem2D))
