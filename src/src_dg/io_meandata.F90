...
CASE ('cor     ')
    call def_stream(elem2D, myDim_elem2D, 'cor',    'coriolis coefficient', 'rad/s',    coriolis(:),                 io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
    
CASE ('SB_EOF1     ')
    call def_stream(elem2D, myDim_elem2D, 'SB_EOF1',    'SB_EOF1', 'n/a',    SB_EOF1(:),                 io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
    
CASE ('cor_nod     ')
    call def_stream(nod2D, myDim_nod2D, 'cor_nod',    'coriolis coefficient', 'rad/s',    coriolis_node(:),                 io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)    
CASE ('heat_flux     ')
    call def_stream(nod2D, myDim_nod2D, 'heat_flux',    'heat flux', 'W/s^2',    heat_flux(:),                 io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)

CASE ('t_star     ')
    call def_stream(nod2D, myDim_nod2D, 't_star',    'air temperature', 'C',    t_star(:),                 io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
    
CASE ('qsr     ')
    call def_stream(nod2D, myDim_nod2D, 'qsr',    'solar radiation', 'W/s^2',    qsr_c(:),                 io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
...
CASE ('e_tu         ')
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'e_tu',         'energy tendency_u','m2/s3',  e_t(1,:,:),            io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
CASE ('e_tv         ')
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'e_tv',         'energy tendency_v','m2/s3',  e_t(2,:,:),            io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
CASE ('w         ')
...
CASE ('v_dis_tend')
    if(visc_option==5 .OR. visc_option==8) then
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'v_dis_tend',    'horizontal velocity viscosity tendency', 'm/s', UV_dis_tend(2,:,:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
    end if    
       
CASE ('u_back_tend')
    if(visc_option==5 .OR. visc_option==8) then    
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'u_back_tend',    'horizontal velocity backscatter tendency', 'm2/s2', UV_total_tend(1,:,:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
    end if
CASE ('v_back_tend')
    if(visc_option==5 .OR. visc_option==8) then    
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'v_back_tend',    'horizontal velocity backscatter tendency', 'm2/s2', UV_total_tend(2,:,:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
    end if

CASE ('EOF_check') 
    if(visc_option==5 .OR. visc_option==8) then
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'EOF_check',    'EOF_check', 'm2/s2', EOF_check(:,:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh) 
    end if


CASE ('uke_dis') 
    if(visc_option==5 .OR. visc_option==8) then
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'uke_dis',    'UKE dissipation', 'm2/s2', uke_dis(:,:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh) 
    end if    
    
CASE ('uke_back') 
    if(visc_option==5 .OR. visc_option==8) then
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'uke_back',    'UKE backscatter', 'm2/s2', uke_back(:,:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh) 
    end if 
    
    
CASE ('uke_adv') 
    if(visc_option==5 .OR. visc_option==8) then
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'uke_adv',    'UKE advection', 'm2/s2', uke_adv(:,:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh) 
    end if 
    
CASE ('uke') 
    if(visc_option==5 .OR. visc_option==8) then
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'uke',    'UKE', 'm2/s2', uke(:,:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh) 
    end if     
    
 
CASE ('tr_hor_adv')
    call def_stream((/nl-1, nod2D/),  (/nl-1, myDim_nod2D/),  'tr_hor_adv',      'tracer horizontal advection', 'C/m2/s2',      del_ttf_advhoriz(:,:),             io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
...
CASE ('UV_rhsAB_u')
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'UV_rhsAB_u',         'advectionAB_u','1/m2/s',  UV_rhsAB(1,:,:),            io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
CASE ('UV_rhsAB_v')
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'UV_rhsAB_v',         'advectionAB_v','1/m2/s',  UV_rhsAB(2,:,:),            io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
CASE ('UV_pr_cor_check')
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'UV_pr_cor_check',         'UV_pr_cor_check','m/s',  UV_pr_cor_check(1,:,:),            io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
CASE ('UV_w_check')
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'UV_w_check',         'UV_w_check','m/s',  UV_w_check(1,:,:),            io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)    
    
    
    
CASE ('UV_adv_u')
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'UV_adv_u',         'advection_u','1/m2/s',  UV_adv(1,:,:),            io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
CASE ('UV_adv_v')
    call def_stream((/nl-1, elem2D/), (/nl-1, myDim_elem2D/), 'UV_adv_v',         'advection_v','1/m2/s',  UV_adv(2,:,:),            io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
...
!3D
  if (ldiag_energy) then
     ...
     if (sel_forcvar(11)==0) call def_stream(elem2D, myDim_elem2D,   'tx_sur',     'zonal wind stress to ocean',     'm/s2', stress_surf(1, 1:myDim_elem2D),1, 'm', i_real4, mesh) ; sel_forcvar(11)=0
     if (sel_forcvar(12)==0) call def_stream(elem2D, myDim_elem2D,   'ty_sur',     'meridional wind stress to ocean','m/s2', stress_surf(2, 1:myDim_elem2D),1, 'm', i_real4, mesh) ; sel_forcvar(12)=0
  end if
...
!!PS   call def_stream((/nl,nod2D/), (/nl,myDim_nod2D/),'sw_3d' , 'penetrated SWR'        , '', sw_3d       , 1, 'm', i_real4, mesh)
  ...
  !end if
    ...
    if (ldiag_curl_vel3) then
        call def_stream((/nl-1, nod2D/),  (/nl-1, myDim_nod2D/),  'curl_u',     'relative vorticity',          '1/s',   curl_vel3(:,:),                   1, 'd', i_real4, mesh)
