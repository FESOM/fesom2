...
allocate(UV_pr_cor_check(2,nl-1, elem_size))
allocate(UV_w_check(2,nl-1, elem_size))
allocate(UV_adv(2,nl-1, elem_size))
allocate(Visc(nl-1, elem_size))
allocate(e_t(2, nl-1, elem_size))
e_t          = 0.0_WP

allocate(chl(myDim_nod2D+eDim_nod2D))
allocate(sw_3d(nl,myDim_nod2D+eDim_nod2D))
chl=0.1_WP
...
allocate(uke_dis(nl-1,elem_size), uke_back(nl-1,elem_size), uke_adv(nl-1,elem_size)) 
allocate(uke_dif(nl-1,elem_size))
allocate(uke_rhs(nl-1,elem_size), uke_rhs_old(nl-1,elem_size))
allocate(EOF_check(nl-1,elem_size))

allocate(UV_dis_tend(2,nl-1,elem_size), UV_back_tend(2,nl-1,elem_size), uke_adv_tend(nl-1,elem_size))
!allocate(SB_l(1))
!allocate(SB_s(1))
!allocate(SB_EOF1(elem_size))
allocate(SB_PC1(1), SB_PC2(1), SB_PC3(1), SB_PC4(1), SB_PC5(1), SB_PC6(1), SB_PC7(1), SB_PC8(1))
allocate(SB_PC9(1), SB_PC10(1), SB_PC11(1), SB_PC12(1), SB_PC13(1), SB_PC14(1), SB_PC15(1), SB_PC16(1), SB_PC17(1))
