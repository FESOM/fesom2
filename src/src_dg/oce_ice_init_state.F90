...
subroutine initial_state_test(mesh)
  ...
  integer                            :: elevation, strat, cooling, tperturb, wind
...
Tsurf=tr_arr(1,:,1)
  ...
  DO elem=1, myDim_elem2D
     ...
     stress_surf(1,elem)=-0.1 *cos(pi*(lat-30.0*rad)/(15.0*rad)) !(8.83*rad))  
  ...
  DO elem=1, myDim_elem2D
     ...
     stress_surf(1,elem)=0.2 *cos(pi*(lat-35.0*rad)/(1250000.0/r_earth))* &
...
!	tr_arr(nz,n,1)=(25.-0.5e-5*r_earth*dst)*exp(Z(nz)/800)
  ...
  if (wind==0) then
   DO elem=1, myDim_elem2D
     ...
     stress_surf(1,elem)=-0.5 *cos(pi*(lat-30.0*rad)/(15*rad))  
...
! ====================================================================
subroutine initial_state_test2(mesh)
  use mod_mesh
  USE o_ARRAYS
  USE o_PARAM
  USE g_PARSUP
...
end subroutine initial_state_test2
! ====================================================================
...
!	tr_arr(nz,n,1)=(25.-0.5e-5*r_earth*dst)*exp(Z(nz)/800)
  ...
  if (wind==0) then
   DO elem=1, myDim_elem2D
     call elem_center(elem, lon, lat, mesh)
     stress_surf(1,elem)=-0.5 *cos(pi*(lat-30.0*rad)/(10*rad))  
      ! 40 is the south boundary of the box
  END DO
  end if
