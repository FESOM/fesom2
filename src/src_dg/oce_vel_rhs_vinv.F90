...
DO elem=1,  myDim_elem2D          !! P (b)  elem=1,elem2D 
   ...
   DO nz=1,nlevels(elem)-1
     ...
     !print *, maxval(UV_rhsAB(1,1,:))
      
      
   END DO   
END DO

!UV_pr_cor_check = UV_rhsAB

!write(*,*) mype, 'hello', maxval(UV_rhsAB(1,1,:)), minval(UV_rhsAB(1,1,:))
! =======================
...
!     w=min(abs(w), 0.0001)*sign(1.0_WP, w)
  ...
  !if(mype==0) write(*,*) 'UV_rhsAB_w ', maxval(UV_rhsAB(1,1,:)), minval(UV_rhsAB(1,1,:))
...
!write(*,*) mype, 'UV_rhs_pr_cor', maxval(UV_rhsAB(1,1,:)), minval(UV_rhsAB(1,1,:))
