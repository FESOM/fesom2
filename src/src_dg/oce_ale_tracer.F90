...
#include "associate_mesh.h"
    ...
    do n=1, myDim_nod2D 
        ...
        !add a randow shock to temperature after 1 year of simulation
        
        
        !if ((daynew == ndpyr) .and. (timenew==86400.)) then
        !if ((daynew == 1) .and. (timenew==86400.)) then
        !    call myrandom(seed, ran)
        !    tr_arr(1,n,1)=tr_arr(1,n,1)*(1+ran*0.1)
        !else
        !    tr_arr(1,n,1)=tr_arr(1,n,1)
        !endif
    end do
...
!            end if 
        ...
        if ((use_sw_pene) .and. (tr_num==1) .and. (.not. toy_ocean)) then
        ...
        !if (mype==0) write(*,*) "heat_flux_oce_ale", heat_flux(100)
