...
!YY: use Vcalc instead of Vdet_a, only needed for calculating calc_diss
    wF(2:Nn+1,ivdet) = Vcalc * abs(zbar_n(2:Nn+1)) + VDet
end if
...
!-Reading Iron--------------------------------------------------------------------
        if (mype==0 .and. my_fesom_group == 0) write(*,*) 'Updating Iron restoring data for month ', i,' from', Ironname    !OG 
...
!     CO2filename = trim(REcoMDataPath)//CO2file//'.nc'
...
!	! data
     ...
     AtmCO2(:)=ncdata(:)
     deallocate(ncdata)
     if (mype==0 .and. my_fesom_group == 0) write(*,*),'Current carbon year=',currentCO2year !OG
     if (mype==0 .and. my_fesom_group == 0) write(*,*),'Atm CO2=', AtmCO2 !OG
...
!   Control output of atmospheric CO2 values
 ...
 ! Aeolian nitrogen deposition
    if (useAeolianN) then
      ...
      if (mype==0 .and. my_fesom_group == 0) write(*,*) 'Updating Nitrogen deposition data for month ', i   !OG  
...
!        num_sec_in_month=num_sec_in_month*86400
           ...
           if (mype==0 .and. my_fesom_group == 0) write(*,*) 'Updating riverine restoring data for month ', i,' from ', Riverfilename     !OG
...
!        num_sec_in_month=num_sec_in_month*86400
           ...
           if (mype==0 .and. my_fesom_group == 0) write(*,*) 'Updating erosion restoring data for month ', i,' from ', Erosionfilename  !OG   
