...
! ======================================================================================
  ...
  real(kind=8),  allocatable :: CO2_watercolumn(:), pH_watercolumn(:),     pCO2_watercolumn(:), HCO3_watercolumn(:) 
  real(kind=8),  allocatable :: CO3_watercolumn(:), OmegaC_watercolumn(:), kspc_watercolumn(:), rhoSW_watercolumn(:)
...
if (recom_debug .and. mype==0) print *, achar(27)//'[36m'//'     --> bio_fluxes'//achar(27)//'[0m'
  call bio_fluxes(tracers, partit, mesh)       !<  alkalinity restoring/ virtual flux is possible
...
!     Temp(1:nzmax) = tr_arr(1:nzmax, n, 1)
     ...
     !!---- Surface salinity
!     Sali = tr_arr(1,       n, 2)
!     Sali_depth(1:nzmax)= tr_arr(1:nzmax, n, 2)
...
!  call exchange_nod(DenitBen, partit)	
  ...
  call exchange_nod(CO23D, partit)       ! NEW ms
  call exchange_nod(pH3D, partit)        ! NEW ms
  call exchange_nod(pCO23D, partit)      ! NEW ms
  call exchange_nod(HCO33D, partit)      ! NEW ms
  call exchange_nod(CO33D, partit)       ! NEW ms
  call exchange_nod(OmegaC3D, partit)    ! NEW ms
  call exchange_nod(kspc3D, partit)      ! NEW ms
  call exchange_nod(rhoSW3D, partit)	 ! NEW ms	
...
!     relax_alk(n)=surf_relax_Alk*(Alk_surf(n)-tr_arr(1,n,2+ialk)) ! 1 temp, 2 salt
     relax_alk(n)=surf_relax_Alk*(Alk_surf(n)-tracers%data(2+ialk)%values(1, n)) ! 1 temp, 2 salt
  end do
