...
!************************* READ SURFACE BOUNDARY FILES *********************************			
  ...
  if (useRivers) then
    call River_input(mesh)      !! --> calculate river input for Fe
  else
    if (recom_debug .and. mype==0) print *, achar(27)//'[36m'//'     --> not call River_input'//achar(27)//'[0m'
  end if
...
!!   We should consider air pressure provided by ECHAM.
     ...
     if (use_MEDUSA .and. (sedflx_num .ne. 0)) then
!!-----------------------------------------------------------------------
!! CV: diffusive fluxes out of sediment have been shifted from recom_sms.F90
!! to oce_ale_tracer.F90, so LocSed is not needed anymore       
!!-----------------------------------------------------------------------
!!        LocSed(1:sedflx_num)    = GloSed(n,1:sedflx_num)
!!-----------------------------------------------------------------------
     else
...
!      C(1:nzmax, 23)    = DIC_13
     ...
     LocRiverDIN          = 0.d0 !RiverDIN2D(n)/vol 
     LocRiverDON          = 0.d0 !RiverDON2D(n)/vol
     LocRiverDOC          = 0.d0 !RiverDOC2D(n)/vol
     LocRiverDSi          = 0.d0 !RiverDSi2D(n)/vol
...
!******************************** RECOM FORCING ****************************************
     ...
     if (use_MEDUSA) then
!CV: This is now done in oce_ale_tracers.F90
!     SinkFlx(n,:)                 = LocSinkFlx(:)      ! updating sinking flux into benthos
   if (sedflx_num .eq. 0) then
...
!        tr_arr(1:nzmax, n, 25:40)    = C(1:nzmax,23:38)
        ...
        if (ciso_14) GloCO2flux_seaicemask_14(n) = co2flux_seaicemask_14(1) !  [mmol/m2/s]
     end if
     GloHplus(n)                  = hplus
     AtmFeInput(n)                = FeDust
     AtmNInput(n)                 = NDust 
...
!CV This is now done in oce_ale_tracer.F90  
!  if(use_MEDUSA) then
!   do n=1, bottflx_num
!    call exchange_nod(SinkFlx(:,n))
!   end do
!  end if ! use_MEDUSA
...
!  end if
  ...
  do n=1, myDim_nod2D+eDim_nod2D
     relax_alk(n)=surf_relax_Alk*(Alk_surf(n)-tr_arr(1,n,5))
  end do
