...
!! *** General constants ***
  ...
  Integer :: idin = 1, idic = 2, ialk = 3, iphyn = 4, iphyc = 5, &
             ...
             ioxy= 22, izoo2n = 23, izoo2c =24
!MB attention: indices 23 and 24 are also used for idic_13 and idic_14 in module REcoM_ciso
...
!!MB TEST: tracer ids for revised remineralization and sinking in oce_ale_tracer.F90
  ...
  integer, dimension(22) :: recom_sinking_tracer_id = (/1007, 1008, 1017, 1021, 1004, 1005, 1020, 1006, &
                                                        1013, 1014, 1016, 1015, 1308, 1321, 1305, 1320, & 
...
!! *** General configuration ***
  ...
  Logical                :: REcoM_Second_Zoo      = .false. ! Decides second zooplankton or no 
  ...
  logical                :: useRivers             = .true.    ! YY: river input of Fe
  ...
  Logical                :: use_MEDUSA            = .true.    ! YY main switch for sediment model
  integer                :: sedflx_num            = 0         ! YY number of sedimentary fluxs from MEDUSA, = 7 if ciso
  Logical                :: add_loopback          = .false.
  real(kind=8)           :: lb_tscale             = 1.d0      ! YY time scale to balance the burial loss
  integer                :: bottflx_num           = 4         ! YY number of stored sinking fluxes from the bottom layer, = 6 if C13 and = 8 if C14
  Logical                :: use_atbox             = .false.   ! MB switch for atmospheric box model for CO2

  namelist /pavariables/ use_REcoM,                       REcoM_restart,           &
                       bgc_num,                           bgc_base_num,          diags3d_num,             &
                       VDet,                              VPhy,                  VDia,                    &
                       ...
                       REcoMDataPath,                     restore_alkalinity,    useRivers,               &
...
!! *** Calcification ***
  ...
  namelist /pacalc/ calc_prod_ratio, calc_diss_guts, calc_diss_rate
!!------------------------------------------------------------------------------
!! *** Benthos ***
...
! riverine input
  ...
  real(kind=8), allocatable,dimension(:)  :: RiverDIN2D, RiverDON2D, RiverDOC2D, RiverDSi2D, RiverFe 
...
Module REcoM_locVar
  ...
  Real(kind=8),allocatable,dimension(:) :: LocSed     ! Flux of solutes from sediment(MEDUSA) to bottom water at curent 2D node
  Real(kind=8),allocatable,dimension(:) :: LocSinkFlx ! sinking flux into benthic layer
