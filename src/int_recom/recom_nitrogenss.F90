...
subroutine recom_nitrogenss(tracers, partit, mesh)
  ...
  real(kind=WP), parameter :: BURIAL_C1 = 0.013_WP   ! Optimisé = 0.02_WP ! Standard = 0.013_WP ! Minimum burial efficiency
  real(kind=WP), parameter :: BURIAL_C2 = 0.53_WP    ! Optimisé = 0.08_WP ! Standard = 0.53_WP ! Maximum additional burial
...
!  end if
  ...
  do n = 1, myDim_nod2D
      ...
      if (enable_3zoo2det) then
          ...
          do nz = nlevels_nod2D_minimum, nl1
              ...
              ! Save sedimentation fluxes
              Ocean_2_Sed_flux(n,1) = tv_n
              Ocean_2_Sed_flux(n,2) = tv_c
              Ocean_2_Sed_flux(n,3) = tv_si
              Ocean_2_Sed_flux(n,4) = tv_calc
  ...
  !do n=1, benthos_num
      ...
      !call exchange_nod(Ocean_2_Sed_flux(:,n), partit)
  !end do
