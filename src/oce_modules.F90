...
! for one, numbers real kinds 1,2,3). On GNU/Intel real32==4 and real64==8, so
! this is a numerical no-op.
#if defined(USE_SINGLE_PRECISION)
integer, parameter            :: WP=real32   ! Single precision
#else
integer, parameter            :: WP=real64   ! Double precision (default)
#endif
! Always-double precision, independent of WP. Use for quantities that must not
! degrade in a single-precision build (time axes, external double APIs, global
