  ...
  isolated_nodes_check: do n_iter = 1, 10
     ...
     do n=1,nod2D
        ...
        if (count(part(ssh_stiff%colind(is:ie)) == part(n)) <= 1) then
           ...
           n_iso = n_iso+1
           print *,'isolated:',n, part(n), part(ssh_stiff%colind(is:ie))
...
! Adjust the load balancing
           ...
           nod_per_partition(2,ne_part(k)) = nod_per_partition(2,ne_part(k)) + nlevels(n)
           nod_per_partition(1,part(n))    = nod_per_partition(1,part(n)) - 1
           nod_per_partition(2,part(n))    = nod_per_partition(2,part(n)) - nlevels(n)
