...
#if defined (__hecubaio)
  logical(kind=c_bool)   :: prune=.false.
#endif
...
#if defined (__hecubaio)
        if (mype==0) then
           call get_prune_state(trim(runid)//c_null_char, prune)
        endif
...
#if defined (__hecubaio)
    if (mype==0 .and. prune) write(*,*) "Prune got called finishing the experiment: ", runid 
#endif
