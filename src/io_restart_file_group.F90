...
module restart_file_group_module
  ...
  type restart_file_group
    private
    type(restart_file_type), public :: files(200)
...
!PS     write(*,*) "--> def_elem_var_3d:", __LINE__, __FILE__
  ...
  subroutine add_file(g, name, must_exist_on_read, mesh_nod2d, mesh_elem2d, mesh_nl, partit, mesh_ncat, mesh_nfbin)
    ...
    if      (present(mesh_ncat)) then
        call f%fesom_file_type%init(mesh_nod2d, mesh_elem2d, mesh_nl, partit, mesh_ncat=mesh_ncat)
    ...
    else if (present(mesh_nfbin)) then
        call f%fesom_file_type%init(mesh_nod2d, mesh_elem2d, mesh_nl, partit, mesh_nfbin=mesh_nfbin)
    else
  ...
  subroutine def_node_var_3d_optional(this, name, longname, units, local_data, mesh, partit, ncat, nfbin)
    use mod_mesh
    class(restart_file_group), intent(inout)            :: this
    character(len=*)         , intent(in)               :: name
    character(len=*)         , intent(in)               :: units, longname
    ...
    type(t_partit)           , intent(in)               :: partit
    integer                  , intent(in)   , optional  :: ncat
    integer                  , intent(in)   , optional  :: nfbin
    ! EO parameters
    if (present(ncat)) then
        !PS add a seprate case for icepack since here i need to write a 2d vertice file
        !PS over the dimension of number of ice thickness classes. Additional input
        !PS parameter ncat
        call add_file(this, name, .false., mesh%nod2d, mesh%elem2d, mesh%nl, partit, mesh_ncat=ncat)
        call this%files(this%nfiles)%specify_node_var(name, longname, units, local_data, ncat)
    else if (present(nfbin)) then
        call add_file(this, name, .false., mesh%nod2d, mesh%elem2d, mesh%nl, partit, mesh_nfbin=nfbin)
        call this%files(this%nfiles)%specify_node_var(name, longname, units, local_data)
    else
        call add_file(this, name, .false., mesh%nod2d, mesh%elem2d, mesh%nl, partit)
        call this%files(this%nfiles)%specify_node_var(name, longname, units, local_data)
