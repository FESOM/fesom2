...
#ifdef __PGI
coord_nod2D(1:,1:)                  			=> mesh%coord_nod2D        
geo_coord_nod2D(1:,1:)              			=> mesh%geo_coord_nod2D    
elem2D_nodes(1:,1:) 					=> mesh%elem2D_nodes
edges(1:,1:)                      			=> mesh%edges              
edge_tri(1:,1:)                   			=> mesh%edge_tri           
elem_edges(1:,1:)                             		=> mesh%elem_edges         
elem_area(1:myDim_elem2D+eDim_elem2D+eXDim_elem2D)	=> mesh%elem_area          
edge_dxdy(1:,1:)                  			=> mesh%edge_dxdy          
edge_cross_dxdy(1:,1:)            			=> mesh%edge_cross_dxdy    
...
elem_neighbors(1:,1:)                         		=> mesh%elem_neighbors     
nod_in_elem2D(1:,1:)      				=> mesh%nod_in_elem2D   ! (maxval(rmax),myDim_nod2D+eDim_nod2D)    
x_corners(1:,1:)          				=> mesh%x_corners   ! (myDim_nod2D, maxval(rmax)) 
y_corners(1:,1:)          				=> mesh%y_corners   ! (myDim_nod2D, maxval(rmax))       
nod_in_elem2D_num(1:myDim_nod2D+eDim_nod2D)             => mesh%nod_in_elem2D_num  
depth(1:myDim_nod2D+eDim_nod2D)                         => mesh%depth              
gradient_vec(1:,1:)                           		=> mesh%gradient_vec       
gradient_sca(1:,1:)                           		=> mesh%gradient_sca       
