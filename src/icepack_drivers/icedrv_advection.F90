...
submodule (icedrv_main) icedrv_advection
  ...
  use icepack_intfc, only: &
       icepack_warnings_flush, icepack_warnings_aborted, icepack_query_tracer_indices,   &
       icepack_query_tracer_flags, icepack_query_parameters, icepack_query_tracer_sizes
  
  implicit none
  
  real(kind=dbl_kind), allocatable, dimension(:)   :: &
       d_tr, trl, rhs_tr,  rhs_trdiv, icepplus, icepminus, mass_matrix
...
#include "associate_mesh.h"
    ...
    gamma = ice%ice_gamma_fct ! It should coinside with gamma in ts_solve_low_order
    ...
    do n = 1, 3   ! three upper nodes; cycle over rows row=elnodes(n)
       icoef(n,n) = -2
    enddo
...
#include "associate_mesh.h"
    ...
    do elem = 1, nx_elem_nh   !! assembling rhs over elements; elem=myList_elem2D(m)
       elnodes = elem2D_nodes(:,elem)
...
#include "associate_mesh.h"
  ...
  subroutine fct_solve_icepack(ice, mesh, trc)
    ...
    call solve_high_order_icepack(mesh, trc) ! uses arrays of low-order solutions as temp storage.
                                             ! It should preceed the call of low order solution.
  ...
  module subroutine tracer_advection_icepack(ice, mesh)
    ...
    integer(kind=int_kind) :: ntrcr, ntrace, narr, nbtrcr, i, nt, nt1, k, nt_Tsfc, nt_qice, &
         nt_qsno, nt_sice, nt_fbri, nt_iage, nt_FY, nt_alvl, nt_vlvl, nt_apnd, nt_hpnd,     &
         nt_ipnd, nt_bgc_Nit, nt_bgc_S
    ...
    real(kind=dbl_kind), dimension(:,:), allocatable :: works
    type(t_mesh), target, intent(in) :: mesh
    type(t_ice), target, intent(in) :: ice
  ...
  subroutine work_to_state (ntrcr, narr, works)
    ...
    real (kind=dbl_kind), dimension(nx,narr), intent (inout) :: works ! work array
    
    ! local variables
    logical (kind=log_kind) :: tr_lvl, tr_pond_lvl, tr_pond_topo
    integer (kind=int_kind) :: &
         nt_alvl, nt_apnd, nt_fbri, nt_Tsfc, nt_qsno, nt_qice, nt_sice, ktherm, &
         k, i, n, it,    & ! counting indices#
         narrays           ! counter for number of state variable arrays
    real (kind=dbl_kind) :: rhos, rhoi, Lfresh, Tsmelt, puny
    real (kind=dbl_kind), dimension(ncat) :: tmp, exc
    real (kind=dbl_kind), parameter :: small=0.000001_dbl_kind
    ...
    if (ktherm == 1) then ! For bl99 themodynamics always ridefine salinity after advection
       do i = 1, nx       ! For each grid cell
          do k = 1, nilyr
  ...
  subroutine state_to_work (ntrcr, narr, works)
    ...
    integer (kind=int_kind), intent(in) :: ntrcr, narr
    real (kind=dbl_kind), dimension(nx,narr), intent (out) :: works ! work array
  
    ! local variables
  
    integer (kind=int_kind) :: &
           nt_alvl, nt_apnd, nt_fbri, nt_Tsfc, nt_qsno, &
           i, n, it   , & ! counting indices
           narrays        ! counter for number of state variable arrays                                                                                   
    logical (kind=log_kind) :: tr_pond_lvl, tr_pond_topo
    real (kind=dbl_kind) :: rhos, Lfresh
    ...
    do n=1, ncat
       ...
       do it = 1, ntrcr
          ...
          elseif (trcr_depend(it) == 2+nt_alvl) then
             do i = 1, nx
                works(i,narrays+it) = aicen(i,n) * trcrn(i,nt_alvl,n) * trcrn(i,it,n)
             enddo
          elseif (trcr_depend(it) == 2+nt_apnd .and. tr_pond_topo) then
             do i = 1, nx
                works(i,narrays+it) = aicen(i,n) * trcrn(i,nt_apnd,n) * trcrn(i,it,n)
             enddo
          elseif (trcr_depend(it) == 2+nt_apnd .and. tr_pond_lvl) then
             do i = 1, nx
                works(i,narrays+it) = aicen(i,n) * trcrn(i,nt_alvl,n) * trcrn(i,nt_apnd,n) * trcrn(i,it,n)
             enddo
          elseif (trcr_depend(it) == 2+nt_fbri) then
             do i = 1, nx
                works(i,narrays+it) = vicen(i,n) * trcrn(i,nt_fbri,n) * trcrn(i,it,n)
             enddo
          endif
       enddo
       narrays = narrays + ntrcr
  ...
  module subroutine cut_off_icepack
    ...
    use icepack_intfc, only: &
         icepack_aggregate, icepack_init_enthalpy, icepack_sea_freezing_temperature
    ...
    real (kind=dbl_kind) :: &
         rhos, Lfresh, cp_ice, cp_ocn, qrd_snow, qrd_ice, Tsfc, exc, depressT, Tmin, &
         T_air_C, hice, puny, Tsmelt, small, rhoi, hpnd_max
       ...
       do i = 1, nx                   ! For each grid point
          ...
          if (vicen(i,n) > small .and. aicen(i,n) > small) then
             ...
             if (flag_cold_ice) then
                ...
                enddo        ! nilyr
                if (vsnon(i,n) > small) then ! Only if there is snow on top of the sea ice
                   do k = 1, nslyr
