...
contains
  ...
  module subroutine ini_mean_icepack_io(mesh)
    ...
    logical (kind=log_kind) :: &
         solve_zsal, skl_bgc, z_tracers, tr_iage, tr_FY, tr_lvl,  &
         tr_aero, tr_pond_topo, tr_pond_lvl, tr_brine, tr_bgc_N,  &
         tr_bgc_C, tr_bgc_Nit, tr_bgc_Sil, tr_bgc_DMS, tr_bgc_chl,&
         tr_bgc_Am, tr_bgc_PON, tr_bgc_DON, tr_zaero, tr_bgc_Fe,  &
         tr_bgc_hum
...
#include "associate_mesh.h"
    ...
    call icepack_query_parameters( &
         solve_zsal_out=solve_zsal, skl_bgc_out=skl_bgc, z_tracers_out=z_tracers, ktherm_out=ktherm)
    call icepack_query_tracer_flags( &
         ...
         tr_bgc_Sil_out=tr_bgc_Sil, tr_bgc_DMS_out=tr_bgc_DMS, tr_bgc_chl_out=tr_bgc_chl,    &
         tr_bgc_Am_out=tr_bgc_Am, tr_bgc_PON_out=tr_bgc_PON, tr_bgc_DON_out=tr_bgc_DON,      &
         tr_zaero_out=tr_zaero, tr_bgc_Fe_out=tr_bgc_Fe, tr_bgc_hum_out=tr_bgc_hum)
    ...
    do i=1, io_listsize
       ...
       case ('lvl       ')
          if (tr_lvl) then
             call def_stream(nod2D, nx_nh, 'alvl', 'ridged ice area', 'none', trcr(:,nt_alvl), &
                  io_list_icepack(i)%freq, io_list_icepack(i)%unit, io_list_icepack(i)%precision, p_partit, mesh)
             call def_stream(nod2D, nx_nh, 'vlvl', 'ridged sea ice volume', 'm', trcr(:,nt_vlvl), &
                  io_list_icepack(i)%freq, io_list_icepack(i)%unit, io_list_icepack(i)%precision, p_partit, mesh)
          end if
       case ('pond_topo ')
       ...
       ! Category dependent (3D) variables
       case ('iagen     ')
          if (tr_iage) then
             call def_stream((/ncat, nod2D/), (/ncat, nx_nh/), 'iage', 'sea ice age', 's', trcrn(:,nt_iage,:), &
                  io_list_icepack(i)%freq, io_list_icepack(i)%unit, io_list_icepack(i)%precision, p_partit, mesh, .true.)
          end if
       case ('FYn       ')
          if (tr_FY) then 
             call def_stream((/ncat, nod2D/), (/ncat, nx_nh/), 'FY', 'first year ice', 'none', trcrn(:,nt_FY,:), &
                  io_list_icepack(i)%freq, io_list_icepack(i)%unit, io_list_icepack(i)%precision, p_partit, mesh, .true.)
          end if
       case ('lvln      ')
          if (tr_lvl) then
             call def_stream((/ncat, nod2D/), (/ncat, nx_nh/), 'alvl', 'ridged sea ice area', 'none', trcrn(:,nt_alvl,:), &
                  io_list_icepack(i)%freq, io_list_icepack(i)%unit, io_list_icepack(i)%precision, p_partit, mesh, .true.)
             call def_stream((/ncat, nod2D/), (/ncat, nx_nh/), 'vlvl', 'ridged sea ice volume', 'm', trcrn(:,nt_vlvl,:), &
                  io_list_icepack(i)%freq, io_list_icepack(i)%unit, io_list_icepack(i)%precision, p_partit, mesh, .true.)
          end if
       case ('pond_topon')
          if (tr_pond_topo) then
             call def_stream((/ncat, nod2D/), (/ncat, nx_nh/), 'apnd', 'melt pond area fraction', 'none', trcrn(:,nt_apnd,:), &
                  io_list_icepack(i)%freq, io_list_icepack(i)%unit, io_list_icepack(i)%precision, p_partit, mesh, .true.)
             call def_stream((/ncat, nod2D/), (/ncat, nx_nh/), 'hpnd', 'melt pond depth', 'm', trcrn(:,nt_hpnd,:), &
                  io_list_icepack(i)%freq, io_list_icepack(i)%unit, io_list_icepack(i)%precision, p_partit, mesh, .true.)
             call def_stream((/ncat, nod2D/), (/ncat, nx_nh/), 'ipnd', 'melt pond refrozen lid thickness', 'm', trcrn(:,nt_ipnd,:), &
                  io_list_icepack(i)%freq, io_list_icepack(i)%unit, io_list_icepack(i)%precision, p_partit, mesh, .true.)
          end if
       case ('pond_lvln ')
          if (tr_pond_lvl) then
       ...
       case ('brinen    ')
          if (tr_brine) then
             call def_stream((/ncat, nod2D/),  (/ncat, nx_nh/), 'fbri', 'volume frac. of ice with dyn. salt', 'none', trcrn(:,nt_fbri,:), &
                  io_list_icepack(i)%freq, io_list_icepack(i)%unit, io_list_icepack(i)%precision, p_partit, mesh, .true.)
          end if
       case ('qicen     ')
          do k = 1,nilyr  ! Separate variable for each sea ice layer
