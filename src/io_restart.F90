...
!_______writing 3D fields________________________________________________
     ...
     call was_error(id); c=1
     if (mype == 0) write(*,*) id%var(id%nvar)%name, 'went well'
  end do
...
!
    ...
    subroutine init_restart_icepack(year, mesh)

        use mod_mesh
        use g_parsup
        use icedrv_kinds
        use icedrv_constants, only: nu_diag
        use icedrv_main,      only: aicen, vicen,  vsnon, &
                                    trcrn, ffracn, dhsn,  &
                                    first_ice_real,       &
                                    ncat, nilyr, nslyr
...
#include "associate_mesh.h"
        ...
        !
        ! Note that at the moment FESOM2 supports only 3D output and restart
        ! (very
        ! suboptimal). The different ice layers are thus splitted in different
        ! arrays
        ! and
        ! reconstructed after the restart. Multidimensional variables would
        ! solve
        ! this.
        ...
        if (tr_pond_cesm) then
            call def_variable_2d(ip_id, 'apnd',  (/nod2D, ncat/), 'melt ponds area',   'none', trcrn(:,nt_apnd,:));
            call def_variable_2d(ip_id, 'vpnd',  (/nod2D, ncat/), 'melt ponds volume', 'm',    trcrn(:,nt_hpnd,:));
        end if

        if (tr_pond_topo) then
            call def_variable_2d(ip_id, 'apnd',  (/nod2D, ncat/), 'melt ponds area',   'none', trcrn(:,nt_apnd,:));
            call def_variable_2d(ip_id, 'vpnd',  (/nod2D, ncat/), 'melt ponds volume', 'm',    trcrn(:,nt_hpnd,:)); 
            call def_variable_2d(ip_id, 'ipnd',  (/nod2D, ncat/), 'melt ponds refrozen lid thickness', 'm',    trcrn(:,nt_ipnd,:));
        end if
        ...
        !
        ! All the other 4D tracers (linked to aerosols and biogeochemistry) are
        ! at the
        ! moment not supported for restart. This might change if someone is
        ! interested
        ! in using the biogeochemistry modules. At this stage, I do not know the
        ! model
