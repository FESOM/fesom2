...
CASE ('BurialN      ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'BurialN','Benthic Burial rate of Nitrogen','mmolN/m2', Burial(1,:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
    end if
    
CASE ('BurialC      ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'BurialC','Benthic Burial rate of Carbon','mmolC/m2', Burial(2,:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
    end if

CASE ('BurialSi     ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'BurialSi','Benthic Burial rate','mmolSi/m2', Burial(3,:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
    end if

CASE ('BurialCalc     ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'BurialCalc','Benthic Burial rate of Calcite','mmolC/m2', Burial(4,:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
    end if
...
CASE ('ReNb     ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'ReNb','benthic N release to the ocean (remineralization)','mmolN/(m2*s)', Sed_2_Ocean_Flux(:,1), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
    end if

CASE ('ReCb     ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'ReCb','benthic C release to the ocean (remineralization + calcification)','mmolC/(m2*s)', Sed_2_Ocean_Flux(:,2), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
    end if

CASE ('ReSib     ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'ReSib','benthic Si release to the ocean (dissolution)','mmolSi/(m2*s)', Sed_2_Ocean_Flux(:,4), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
    end if

CASE ('ReAlkb     ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'ReAlkb','benthic Alk release to the ocean','mmolC-eq/(m2*s)', Sed_2_Ocean_Flux(:,3), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
    end if

CASE ('ReFeb     ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'ReFeb','benthic Fe release to the ocean','mmolFe/(m2*s)', Sed_2_Ocean_Flux(:,5), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
    end if

CASE ('ReO2b     ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'ReO2b','benthic O2 release to the ocean','mmolO2/(m2*s)', Sed_2_Ocean_Flux(:,6), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
    end if
