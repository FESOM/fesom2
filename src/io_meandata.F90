...
subroutine mio_write_nod(mio, entry)
    ...
    do lev=1, size1
        ...
        if (.not. entry%is_elem_based) then
           cerr = mio%write_field(md, entry%local_values_r8_copy(lev, 1:entry%shrinked_size))
        else
           cerr = mio%write_field(md, entry%local_values_r8_copy(lev,   entry%shrinked_indx))
        end if
