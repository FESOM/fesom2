...
!
  ...
  type Meandata
    ...
    real(real64), allocatable :: aux3d_r8(:,:)
...
!3D
  if (ldiag_energy) then
     call def_stream((/nl,   nod2D/),  (/nl,     myDim_nod2D/), 'rhof',     'in-situ density at faces',    'kg/m3',     rhof(:,:),  1, 'm', i_real4, mesh)
     call def_stream((/nl,   nod2D/),  (/nl,     myDim_nod2D/), 'wrhof',    'vertical velocity x density', 'kg/(s*m2)', wrhof(:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/), 'uu',   'u times u', 'm2/s2', u_x_u(:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/), 'uv',   'u times v', 'm2/s2', u_x_v(:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/), 'vv',   'v times v', 'm2/s2', v_x_v(:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl-1,  elem2D/),  (/nl-1,   myDim_elem2D/),'uw',   'u times w', 'm2/s2', u_x_w(:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl-1,  elem2D/),  (/nl-1,   myDim_elem2D/),'vw',   'v times w', 'm2/s2', v_x_w(:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/), 'tu',   'T times u', 'K*m/s', T_x_u(:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/), 'tv',   'T times v', 'K*m/s', T_x_v(:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/), 'su',   'S times u', 'psu*m/s', S_x_u(:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/), 'sv',   'S times v', 'psu*m/s', S_x_v(:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl  , nod2D/),  (/nl  ,   myDim_nod2D/), 'tw',   'T times w', 'K*m/s', T_x_w(:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl  , nod2D/),  (/nl  ,   myDim_nod2D/), 'sw',   'S times w', 'psu*m/s', S_x_w(:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/), 'dudx', 'du/dx',     '1/s',   dudx(:,:),  1, 'm', i_real4, mesh)
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/), 'dudy', 'du/dy',     '1/s',   dudy(:,:),  1, 'm', i_real4, mesh)
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/), 'dvdx', 'dv/dx',     '1/s',   dvdx(:,:),  1, 'm', i_real4, mesh)
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/), 'dvdy', 'dv/dy',     '1/s',   dvdy(:,:),  1, 'm', i_real4, mesh)
     call def_stream((/nl,  elem2D/),  (/nl,     myDim_elem2D/), 'dudz', 'du/dz',     '1/s',   dudz(:,:),  1, 'm', i_real4, mesh)
     call def_stream((/nl,  elem2D/),  (/nl,     myDim_elem2D/), 'dvdz', 'dv/dz',     '1/s',   dvdz(:,:),  1, 'm', i_real4, mesh)
     ...
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/),  'unod',  'horizontal velocity at nodes', 'm/s', Unode(1,:,:), 1, 'm', i_real4, mesh)
     call def_stream((/nl-1, nod2D/),  (/nl-1,   myDim_nod2D/),  'vnod',  'meridional velocity at nodes', 'm/s', Unode(2,:,:), 1, 'm', i_real4, mesh)
     ...
     call def_stream((/nl, nod2D/),    (/nl,     myDim_nod2D/),  'wm',  'vertical velocity',   'm/s', Wvel(:,:),     1, 'm', i_real4, mesh)
...
!___________writing 8 byte real _________________________________________ 
  if(entry%accuracy == i_real8) then
    if(mype==entry%root_rank) then
      if(entry%ndim==1) then ! i.e. a 2D field
        if(.not. allocated(entry%aux3d_r8)) allocate(entry%aux3d_r8(size1,size2)) ! todo: change index order here, lvl is first so it will be the wrong order if we dump the whole array to netcdf without transposing it
      else ! i.e. a 3D field
        if(.not. allocated(entry%aux3d_r8)) allocate(entry%aux3d_r8(size1,size2)) ! todo: change index order here, lvl is first so it will be the wrong order if we dump the whole array to netcdf without transposing it
      end if
    end if
    ...
    if(.not. entry%is_elem_based) then
      call gather_real8_nod3D (entry%local_values_r8_copy, entry%aux3d_r8, entry%root_rank, tag, entry%comm)
    else
      call gather_real8_elem3D(entry%local_values_r8_copy, entry%aux3d_r8, entry%root_rank, tag, entry%comm)
    end if
    ...
    if (mype==entry%root_rank) then
      if (entry%ndim==1) then
        ...
        call assert_nf( nf_put_vara_real(entry%ncid, entry%varID, (/1, entry%rec_count/), (/size2, 1/), transpose(entry%aux3d_r8)), __LINE__)
      else if (entry%ndim==2) then
#ifndef TRANSPOSE_OUTPUT
        call assert_nf( nf_put_vara_real(entry%ncid, entry%varID, (/1, 1, entry%rec_count/), (/size1, size2, 1/), entry%aux3d_r8), __LINE__)
#else
        ! transpose the global aux array when writing all levels at once, otherwise it is transposed in the netcdf file
        ! we can either use transpose(entry%aux3d_r8) or use nf_put_varm with an imap=(/size1,1/) to transpose the data
        ! nf_put_varm was very slow on aleph in a test with dart mesh
        call assert_nf( nf_put_vara_real(entry%ncid, entry%varID, (/1, 1, entry%rec_count/), (/size2, size1, 1/), transpose(entry%aux3d_r8)), __LINE__)
        ...
        ! call assert_nf( nf_put_varm_real(entry%ncid, entry%varID, (/1, 1, entry%rec_count/), (/size2, size1, 1/), stride, imap, entry%aux3d_r8), __LINE__)
#endif
      end if
    end if
