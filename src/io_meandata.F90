    ...
    if (mod(mix_scheme_nmb,10)==7) then
        ...
        if (idemix2_enable_M2) then 
            ...
            if (idemix2_diag_Ecompart) then
                if (allocated(iwe2_M2_reflect_src )) call def_stream((/idemix2_nfbin, nod2d/),(/idemix2_nfbin,myDim_nod2D /), 'iwe2_Em2_reflsrc' , 'M2 coast-reflect. accumulated flux' , 'm^4/s^3'  , iwe2_M2_reflect_src(:,:)  , io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
        ...
        if (idemix2_enable_niw) then 
            ...
            ! optional diagnostic
            if (idemix2_diag_Ecompart) then
                if (allocated(iwe2_niw_reflect_src)) call def_stream((/idemix2_nfbin, nod2d/),(/idemix2_nfbin,myDim_nod2D /), 'iwe2_Eniw_reflsrc', 'niw coast-reflect. accumulated flux', 'm^4/s^3' , iwe2_niw_reflect_src(:,:) , io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
