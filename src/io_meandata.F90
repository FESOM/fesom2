...
CASE ('dMOC      ')
    ...
    if (ldiag_dMOC) then
       ...
       call def_stream((/std_dens_N, elem2D/),  (/std_dens_N, myDim_elem2D/), 'w_diap_diff',     'diagnosed w_diap', 'fluxes', w_diap(:,:),    io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
       call def_stream((/std_dens_N, elem2D/),  (/std_dens_N, myDim_elem2D/), 'dV_diap_dT',      'dV_diap_dT',       'm/s',    dV_diap_dT(:,:),io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
    end if
...
END DO ! --> DO i=1, io_listsize
    ...
    if (ldiag_dMOC .and. sel_dmoc == 0) then
       ...
       call def_stream((/std_dens_N, elem2D/),  (/std_dens_N, myDim_elem2D/), 'w_diap_diff',    'diagnosed w_diap',       'm/s',    w_diap(:,:),            1, 'y', i_real4, partit, mesh)
       call def_stream((/std_dens_N, elem2D/),  (/std_dens_N, myDim_elem2D/), 'dV_diap_dT',     'dV_diap_dT',             'm/s',    dV_diap_dT(:,:),        1, 'y', i_real4, partit, mesh)
    end if
