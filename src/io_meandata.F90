...
CASE ('dens      ')
    call def_stream((/nl-1, nod2D/),  (/nl-1, myDim_nod2D/),  'dens',      'potential density - rho0 ',    'kg/m^3',    density_m_rho0(:,:),             io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)    
CASE ('otracers  ')
    do j=3, num_tracers
...
CASE ('KvN2      ')
    call def_stream((/nl,    nod2D/), (/nl,   myDim_nod2D/),  'KvN2',      'Kv * N^2',  'm2/s3', KvN2(:,:),              io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
CASE ('u         ')
