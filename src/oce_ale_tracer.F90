...
subroutine adv_tracers_muscle_ale(ttfAB, num_ord, do_Xmoment, mesh)
    ...
    integer                  :: nl1, nl2, nl12, ul1, ul2, ul12
...
!!PS             c1=-0.5_WP*( (1.0_WP-num_ord)*c1+vflux*num_ord*( (Tmean1+Tmean2)**do_Xmoment ) )
        ...
        end do ! --> do nz=ul12, nl12
        
        !_______________________________________________________________________
        ! (D) remaining segments on the left or on the right
        !!PS if(nl1>nl2) then --> dont need that if condition
...
#include "associate_mesh.h"
    ...
    do n=1, myDim_nod2D
        ...
        do nz=nzmin+1, nzmax-3
            ...
            !!PS dzjm1    = hnode_new(nz-2,n)
            !!PS dzj      = hnode_new(nz-1,n)
            !!PS dzjp1    = hnode_new(nz,n)
            !!PS dzjp2    = hnode_new(nz+1,n)
        ...
        !!PS tvert(1:nzmax)=0._WP
        tvert(nzmin:nzmax)=0._WP
        ...
        !_______________________________________________________________________
        ! Surface flux
        !!PS tvert(1)= -( tv(1)**do_Xmoment )*Wvel_e(1,n)*area(1,n)
        tvert(nzmin)= -( tv(nzmin)**do_Xmoment )*Wvel_e(nzmin,n)*area(nzmin,n)
        ! Zero bottom flux
        tvert(nzmax)=0.0_WP        
...
#include "associate_mesh.h"
    ...
    do n=1, myDim_nod2D
        ...
        !_______________________________________________________________________
        ! Surface flux
        !!PS tvert(1)= -Wvel_e(1,n)*(ttf(1,n)**do_Xmoment)*area(1,n)
        tvert(nzmin)= -Wvel_e(nzmin,n)*(ttf(nzmin,n)**do_Xmoment)*area(nzmin,n)
        !_______________________________________________________________________
        ! Zero bottom flux
...
!         zbar_n(nzmax)=zbar(nzmax)
        ...
        if (tracer_adv/=2) then
            ...
            !!PS b(1)=b(1)+Wvel_i(1, n)*zinv-min(0._WP, Wvel_i(2, n))*v_adv
            !!PS c(1)=c(1)-max(0._WP, Wvel_i(2, n))*v_adv
