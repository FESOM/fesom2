module io_gather_module
  implicit none
  public init_io_gather, gather_nod2D, gather_real4_nod2D, gather_elem2D, gather_real4_elem2D, gather_real8_nod3D, gather_real8_elem3D, gather_real4_nod3D, gather_real4_elem3D
  private
...
contains
...
#ifdef ENABLE_IOSERVER
    if(ioserver%is_ioserver()) then
      if(.not. allocated(remPtr_elem2D)) allocate(remPtr_elem2D(comm_fesom_npes))
      call mpi_recv(remPtr_elem2D, size(remPtr_elem2D), MPI_INTEGER, 0, 42, comm_fesom_with_ioserver, status, MPIerr)
      
      if(.not. allocated(remList_elem2D)) allocate(remList_elem2D(remPtr_elem2D(comm_fesom_npes)))
      call mpi_recv(remList_elem2D, size(remList_elem2D), MPI_INTEGER, 0, 42, comm_fesom_with_ioserver, status, MPIerr)

      call mpi_recv(rank0Dim_elem2D, 1, MPI_INTEGER, 0, 42, comm_fesom_with_ioserver, status, MPIerr)
      
      allocate(rank0List_elem2D(rank0Dim_elem2D))
      call mpi_recv(rank0List_elem2D, size(rank0List_elem2D), MPI_INTEGER, 0, 42, comm_fesom_with_ioserver, status, MPIerr)
    else if(mype==0) then
      call mpi_send(remPtr_elem2D, size(remPtr_elem2D), MPI_INTEGER, ioserver_rank, 42, comm_fesom_with_ioserver, MPIerr)
      call mpi_send(remList_elem2D, size(remList_elem2D), MPI_INTEGER, ioserver_rank, 42, comm_fesom_with_ioserver, MPIerr)
      call mpi_send(rank0Dim_elem2D, 1, MPI_INTEGER, ioserver_rank, 42, comm_fesom_with_ioserver, MPIerr)
      call mpi_send(myList_elem2D, myDim_elem2D, MPI_INTEGER, ioserver_rank, 42, comm_fesom_with_ioserver, MPIerr)
    end if
#endif
  ...
  ! thread-safe procedure
  subroutine gather_real8_nod3D(arr3D, arr3D_global, root_rank, tag, io_comm)
    use, intrinsic :: ISO_C_BINDING
    use g_PARSUP
    use ioserver_module
    use, intrinsic :: iso_fortran_env, only: real64
    implicit none
    real(real64), target  :: arr3D(:,:)
    real(real64), intent(out) :: arr3D_global(:,:)
    integer, intent(in) :: root_rank ! rank of receiving process
    integer, intent(in) :: tag
    integer io_comm
    ! EO args
    ...
    integer :: remote_node_count = -1
!    real(real64), allocatable :: sendbuf(:,:)
    real(real64), pointer :: sb_ptr(:)
!    real(real64), allocatable :: recvbuf(:) ! todo: alloc only for root_rank
!    real(real64), allocatable :: recvbufs(:,:)
    real(real64), target, allocatable :: recvbuf3D(:,:)
    real(real64), target, allocatable :: rb(:)
    real(real64), pointer :: rb_ptr(:)
    integer                   :: req(npes-1)
    integer :: request_index
    integer :: mpi_precision = MPI_REAL
    integer lvl
...
!    real(real32), allocatable :: recvbufs(:,:)
  ...
  ! thread-safe procedure
  subroutine gather_real8_elem3D(arr3D, arr3D_global, root_rank, tag, io_comm)
    use, intrinsic :: ISO_C_BINDING
    use g_PARSUP
    use, intrinsic :: iso_fortran_env, only: real64
    implicit none
    real(real64), target, intent(in)  :: arr3D(:,:)
    real(real64), intent(out) :: arr3D_global(:,:)
    integer, intent(in) :: root_rank ! rank of receiving process
    integer, intent(in) :: tag
    integer io_comm
...
!    real(real64), allocatable              :: sendbuf(:)
    ...
    include "io_gather_elem3d.inc"
  end subroutine


  ! thread-safe procedure
  subroutine gather_real4_elem3D(arr3D, arr3D_global, root_rank, tag, io_comm)
    use, intrinsic :: ISO_C_BINDING
    use g_PARSUP
    use, intrinsic :: iso_fortran_env, only: real32
    implicit none
    real(real32), target, intent(in)  :: arr3D(:,:)
    real(real32), intent(out) :: arr3D_global(:,:)
    integer, intent(in) :: root_rank ! rank of receiving process
    integer, intent(in) :: tag
    integer io_comm
