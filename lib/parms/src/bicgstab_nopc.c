...
typedef struct bicgs_nopc_data {
  int neigs;
} *bicgstab_nopc_data;

#define TINY 1.0e-20
int parms_bicgstab_nopc(parms_Solver self, FLOAT *y, FLOAT *x)
{
...
/* --- first permute x and y for pARMS matrix structure ------*/
	...
	for (i = 0; i < nloc; i++) x[i] += alpha * p[i];
	...
	for (i = 0; i < nloc; i++) x[i] += sigma * r[i];
...
static int bicgstab_nopc_getresidual(parms_Solver self, FLOAT *y, FLOAT *x, FLOAT *res)
{
...
static int bicgstab_nopc_getresidualnorm2(parms_Solver self, FLOAT *y, FLOAT *x, REAL *rnorm)
{
...
static int bicgstab_nopc_free(parms_Solver *self)
{
  /*dummy*/
  return 0;
}

static int bicgstab_nopc_view(parms_Solver self, parms_Viewer v)
{
  FILE *fp;
  char *name, *iluname;
  int dim;
  ...
  fprintf(fp, "Solver type = BiConjugate Gradient Stabilized Method w/o preconditioner (BICGS_NOPC) \n");
  
  fprintf(fp, "maxits = %d \n", self->maxits);
  fprintf(fp, "Relative tolerance = %-8.2e \n", self->tol);

  fprintf(fp, "Global Preconditioner: NONE %s\n");
  fprintf(fp, "Local Preconditioner: NONE %s\n");
...
static int bicgstab_nopc_setksize(parms_Solver self, int restart)
{
  /*dummy*/
  return 0;
}

static int bicgstab_nopc_setneig(parms_Solver self, int neigs)
{
  /*dummy*/
  return 0;
}

static struct parms_Solver_ops parms_bicgstab_nopc_sol = {
  parms_bicgstab_nopc,
  bicgstab_nopc_getresidual,
  bicgstab_nopc_getresidualnorm2,
  bicgstab_nopc_setksize,
  bicgstab_nopc_setneig,
  bicgstab_nopc_free,
  bicgstab_nopc_view
};


/** Create the BICGS_NOPC solver. 
 *
 *  \param self A parms_Solver object.
 *  \return 0 on success.
 */
int bicgstab_nopc_create(parms_Solver self)
{
  PARMS_MEMCPY(self->ops, &parms_bicgstab_nopc_sol, 1);		
  return 0;
}
