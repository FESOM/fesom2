...
- In coupled runs the received runoff is split into liquid and solid parts, available as the output streams ``runoff_liquid`` and ``runoff_solid`` in addition to the total ``runoff``.
