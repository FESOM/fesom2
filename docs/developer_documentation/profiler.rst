...
with the accumulated wall time, the call count and the mean, and two separate
measures of how badly the work is distributed:
...
Reading the two together tells you what kind of imbalance you have. A high
range imbalance with a low standard imbalance points at one straggling rank,
typically a partition with an unusual number of surface nodes or cavity
points. Both being high points at a systematic problem with the partitioning
or with a physical process that is active in one region only.
